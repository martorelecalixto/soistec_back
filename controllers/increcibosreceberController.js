...
const incReciboRec = async (req, res) => {
  ...
  while (!atualizado) {
    ...
    try {
      ...
      const checkColumnSql = `
        ...
        WHERE TABLE_NAME = 'IncReciboRec' AND COLUMN_NAME = @coluna
      `;
      request.input('coluna', sql.NVarChar, `id_${idempresa}`);
      const checkResult = await request.query(checkColumnSql);

      if (checkResult.recordset.length === 0) {
        const addColumnSql = `ALTER TABLE IncReciboRec ADD id_${idempresa} INT`;
        await request.batch(addColumnSql);
        const updateSql = `UPDATE IncReciboRec SET id_${idempresa} = 1`;
        await request.batch(updateSql);
      } else {
        const updateSql = `UPDATE IncReciboRec SET id_${idempresa} = id_${idempresa} + 1`;
        await request.batch(updateSql);
      }
      ...
      const result = await pool.request().query(`SELECT id_${idempresa} AS valor FROM IncReciboRec`);
