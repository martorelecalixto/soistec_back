...
const incTituloRec = async (req, res) => {
  ...
  while (!atualizado) {
    ...
    try {
      ...
      const checkColumnSql = `
        ...
        WHERE TABLE_NAME = 'IncTituloRec' AND COLUMN_NAME = @coluna
      `;
      request.input('coluna', sql.NVarChar, `id_${idempresa}`);
      const checkResult = await request.query(checkColumnSql);

      if (checkResult.recordset.length === 0) {
        const addColumnSql = `ALTER TABLE IncTituloRec ADD id_${idempresa} INT`;
        await request.batch(addColumnSql);
        const updateSql = `UPDATE IncTituloRec SET id_${idempresa} = 1`;
        await request.batch(updateSql);
      } else {
        const updateSql = `UPDATE IncTituloRec SET id_${idempresa} = id_${idempresa} + 1`;
        await request.batch(updateSql);
      }
      ...
      const result = await pool.request().query(`SELECT id_${idempresa} AS valor FROM IncTituloRec`);
